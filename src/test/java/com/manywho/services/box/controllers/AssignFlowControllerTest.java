...
public class AssignFlowControllerTest extends BoxServiceFunctionalTest {
    ...
    public void testAssignFlowToWebhook() throws IOException, URISyntaxException {
        ...
        requestIntersectorTests.addApiResponse(createBoxApiResponse("flow-assignment/with-version-id/box-response/create-webhook.json", 200));
        ...
        mockJedis.set(userCredentials, getJsonFormatFileContent("flow-assignment/with-version-id/db/credentials.json"));

        Response responseMsg = target("/trigger-assign/flow").request()
                .headers(headers)
                .post(getServerRequestFromFile("flow-assignment/with-version-id/request.json"));

        //check the response is right
        assertJsonSame(
                getJsonFormatFileContent("flow-assignment/with-version-id/response.json"),
                getJsonFormatResponse(responseMsg)
        );

        assertEquals("\"123456789\"", mockJedis.get(userTokenKey));
        assertJsonSame(getJsonFormatFileContent("flow-assignment/with-version-id/db/credentials-after-action.json"),
    ...
    @Test
    public void testAssignFlowWithoutVersionIdToWebhook() throws IOException, URISyntaxException {
        MultivaluedMap<String,Object> headers = new MultivaluedHashMap<>();
        headers.add("Authorization", AuthorizationUtils.serialize(getAuthenticatedWho()));

        requestIntersectorTests.addApiResponse(createBoxApiResponse("flow-assignment/without-version-id/box-response/create-webhook.json", 200));
        String userCredentials = "service:box:user:123456789:credentials";
        String userTokenKey = "service:box:user:token:12345678";
        String flowHeaderKey = "service:box:box-userid:123456789:flow-auth-header";

        mockJedis.set(userCredentials, getJsonFormatFileContent("flow-assignment/without-version-id/db/credentials.json"));

        Response responseMsg = target("/trigger-assign/flow").request()
                .headers(headers)
                .post(getServerRequestFromFile("flow-assignment/without-version-id/request.json"));

        //check the response is right
        assertJsonSame(
                getJsonFormatFileContent("flow-assignment/without-version-id/response.json"),
                getJsonFormatResponse(responseMsg)
        );

        assertEquals("\"123456789\"", mockJedis.get(userTokenKey));
        assertJsonSame(getJsonFormatFileContent("flow-assignment/without-version-id/db/credentials-after-action.json"),
                mockJedis.get(userCredentials));
